/**
 * Integration tests for rubric routes.
 *
...
function makeToken(role = 'student') {
  return jwt.sign(
    ...
    SECRET,
    { expiresIn: '1h', jwtid: 'jti-rubric-test' },
  );
}
...
describe('GET /rubrics', () => {
  ...
  it('should return rubric for authenticated user with valid query', async () => {
    mockAuth();
    const token = makeToken();

    // Controller returns single rubric row or 404
    mockClient.query.mockResolvedValue({
      rows: [{ rubric_id: 'r-1', rubric_type: 'file_review', levels: [], updated_at: new Date().toISOString() }],
    });

    const res = await request(app)
      .get('/rubrics?rubric_type=file_review')
      .set('Cookie', `token=${token}`);

    expect(res.status).toBe(200);
    expect(res.body.rubric_type).toBe('file_review');
  });
});
...
describe('POST /rubrics', () => {
  it('should return 403 for student role', async () => {
    ...
    const res = await request(app)
      .post('/rubrics')
      .set('Cookie', `token=${token}`)
      .send({ rubric_type: 'file_review', criteria: [] });
